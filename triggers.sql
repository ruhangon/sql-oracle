...
-- exemplo de trigger
-- se salário recebe aumento de mais de 500 reais mostra mensagem
create or replace trigger controla_salario
after update of sl_vendedor on vendedor
for each row
declare
wdif_sal vendedor.sl_vendedor%type;
begin
wdif_sal := ((:new.sl_vendedor)-(:old.sl_vendedor));
if (wdif_sal>500) then
dbms_output.put_line('o novo salário teve um acréscimo de '||(wdif_sal));
end if;
end;
/

-- outro exemplo de trigger
-- se remove venda remove também suas parcelas
create or replace trigger controla_remocao
before delete on venda
for each row
declare
wcd_ven venda.cd_venda%type;
begin
wcd_ven := (:old.cd_venda);
dbms_output.put_line('venda '||wcd_ven||' será deletada, junto com suas parcelas');
delete from parcela where cd_venda = wcd_ven;
end;
/
